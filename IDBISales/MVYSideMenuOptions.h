...
@property (nonatomic, assign) float menuViewOverlapWidth __attribute__((deprecated("Use menuFrame attribute in MVYSideMenuController instead")));

/** Pan gesture bezel width in pixel when menu is closed. Default 20.0 */
@property (nonatomic, assign) float bezelWidth;

/** Content view scale factor when menu is opened. 0.0 max scale, 1.0 disable scale. Default 0.96 */
@property (nonatomic, assign) float contentViewScale;

/** Content view opacity when menu is opened. 1.0 opaque 0.0 clear. Default 0.4 */
@property (nonatomic, assign) float contentViewOpacity;
...
@property (nonatomic, assign) Size shadowOffset;
